...
test('bad task deletion', async () => {
    ...
    const task_id = await createTask(
        ...
        task_status, [task_creator, user2_id], task_project, task_description, task_estimated_days
    )
...
test('correct task deletion', async () => {
    ...
    const user = await getConnection().getRepository(User).find({where : {email : "asd@gmail.com"}});
    const task_creator = user[0].id;
    const user2 = await getConnection().getRepository(User).find({where : {email : "asd@gmail.com"}});
    const user2_id = user2[0].id;
    const task_project: string = null;
    const task_title = "title";
    const task_deadline = new Date();
    ...
    tasks = await getConnection().getRepository(Task).find({where : {id : task_id}});
