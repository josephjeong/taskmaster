...
test('invalid ids test', async () => {
    try {await (getTask("","asd"))} catch (e) {
        expect(e.code).toBe("getTask/invalid_user_id");
        expect(e.message).toBe("user_id provided is null/undefined or empty string");
    }
    try {await (getTask(undefined,"asd"))} catch (e) {
        expect(e.code).toBe("getTask/invalid_user_id");
        expect(e.message).toBe("user_id provided is null/undefined or empty string");
    }
    try {await (getTask(null,"asd"))} catch (e) {
        expect(e.code).toBe("getTask/invalid_user_id");
        expect(e.message).toBe("user_id provided is null/undefined or empty string");
    }
    try {await (getTask("s",""))} catch (e) {
        expect(e.code).toBe("getTask/invalid_task_id");
        expect(e.message).toBe("task_id provided is null/undefined or empty string");
    }
    try {await (getTask("s",null))} catch (e) {
        expect(e.code).toBe("getTask/invalid_task_id");
        expect(e.message).toBe("task_id provided is null/undefined or empty string");
    }
    try {await (getTask("d", undefined))} catch (e) {
        expect(e.code).toBe("getTask/invalid_task_id");
        expect(e.message).toBe("task_id provided is null/undefined or empty string");
    }
    try {await (getTask("d", "s"))} catch (e) {
...
test('unconnected test', async () => {
    ...
    try {await (getTask(task_creator2, task_id))} catch (e) {
        expect(e.code).toBe("getTask/no_perm");
        expect(e.message).toBe("You are unauthorised, must be connected to an assignee of the task");
    }
