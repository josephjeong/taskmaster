...
it('logs in a user correctly', async () => {
    ...
    const payload1 = await decodeJWTPayload(res1.body.token);
    ...
    const payload2 = await decodeJWTPayload(res2.body.token);
...
it('requests user details correctly', async () => {
    ...
    const user_id = (await decodeJWTPayload(res1.body.token)).id;

    const res2 = await req.get(`/users/details/${user_id}`);
    const body : UserDetails = res2.body
...
it('requests "my" details correctly', async () => {
    ...
    const res2 = await req.get('/users/me').set({jwt: res1.body.token});
    const body : UserDetails = res2.body
...
it('updates user details correctly', async () => {
    ...
    //update users
    await req.post('/users/update')
        .set({jwt: res1.body.token})
        .send({
            changes: {
    ...
    const res3 = await req.get('/users/me').set({jwt: res1.body.token});
    const body : UserDetails = res3.body
