...
export async function deleteTask(
    ...
    if (to_remove_assigns.length == 0) throw new ApiError("deleteTask/invalid_task_id", "task with this id does not exist");
    if (to_remove_assigns[0].task.creator.id !== deletor_id) {
        throw new ApiError("deleteTask/invalid_delete", "deletor_id must match task creator's id to delete");
    }
