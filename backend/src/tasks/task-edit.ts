...
import { validAssignees, deleteAssignment, deleteAssignmentByTaskAndUserId } from "./task-helpers";
...
export async function editTask(
    ...
    if (!task_id)
        throw new ApiError("editTask/invalid_task_id", "task_id is null/undefined or empty string");
    
    if (!editor)
        throw new ApiError("editTask/invalid_editor", "editor id is null/undefined or empty string");
    ...
    if (!((title && title.trim().length > 0) || deadline || status || (description !== undefined && description !== null) || 
        (estimated_days !== undefined && estimated_days !== null) || (add_assignees && add_assignees.length > 0) || 
        (remove_assignees && remove_assignees.length > 0))) { // && group
        throw new ApiError("editTask/bad_params", "ensure at least one editable field is defined or not empty");
    }
    
    // check valid status
    if (status && !Object.values(Status).includes(status)) {
        throw new ApiError("editTask/invalid_status", 'Status enum is {NOT_STARTED = "TO_DO", IN_PROGRESS = "IN_PROGRESS", BLOCKED = "BLOCKED", COMPLETED = "DONE"}');
    }
    
    if (deadline && !(deadline instanceof Date))
        throw new ApiError("editTask/invalid_deadline", "deadline provided is not a Date");
    
    // ensure deadline in the future
    if (deadline && deadline.getTime() <= Date.now())
        throw new ApiError("editTask/invalid_deadline", "deadline must be in the future");
    
    // ensure estimated_days is positive
    if (estimated_days !== null && estimated_days !== undefined && estimated_days < 0) {
        throw new ApiError("editTask/invalid_estimated_days", "estimated_days must be >= 0");
    }
    
    // get task
    let tasks = await getConnection().getRepository(Task).find({where : {id : task_id}, relations: ["creator"], loadRelationIds: true});
    
    if (tasks.length === 0) 
        throw new ApiError("editTask/task_id_nonexistent", "task does not exist");
    
    if (tasks.length > 1) 
        throw new ApiError("editTask/duplicate_task_ids", "there are duplicate task ids in db, oh no this is bad");
    
    // check editor is creator of task
    if (tasks[0].creator !== editor) {
        throw new ApiError("editTask/no_perm", "this user cannot edit this task, only it's creator can");
    }
    ...
    // no assignee changes
    if ((!add_assignees && !remove_assignees) || (add_assignees && add_assignees.length === 0 && 
    ...
    if (add_assignees && !(await validAssignees(editor, add_assignees)))
        throw new ApiError("editTask/invalid_add_assignees", "add_assignees must be connected to editor or in same group");
    ...
    if (remove_assignees && !usersAreAssigned(remove_assignees, current_assignees))
        throw new ApiError("editTask/invalid_remove_assignees", "remove_assignees contains id of a user that is not currently assigned to the task");
    
    await getConnection().manager.save(tasks[0]);
    await updateAssignments(add_assignees, remove_assignees, task_id, current_assignees);
    
    // if no assignee assign to creator
    let assignees = getNewAssignees(current_assignees, add_assignees, remove_assignees);
